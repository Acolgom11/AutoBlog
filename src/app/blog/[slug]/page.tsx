...
import { getArticleBySlug, getArticleSlugs, getAllArticles, ArticleResult } from "@/lib/mdx";
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  const articleResult: ArticleResult | null = getArticleBySlug(resolvedParams.slug);
  ...
  const breadcrumbLd = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": data.category,
        "item": `https://www.autoblog.com/category/${data.category.toLowerCase().replace(/ /g, "-")}`
      },
      {
        "@type": "ListItem",
        "position": 3,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <Breadcrumbs
        items={[
          { label: data.category, href: `/category/${data.category.toLowerCase().replace(" ", "-")}` },
